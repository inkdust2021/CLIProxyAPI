...
func (m configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	fields = append(fields, configField{"Fatal Auth Mode", "fatal-auth-enabled", "string", getString(cfg, "fatal-auth-enabled"), nil})
	fields = append(fields, configField{"Fatal Auth Action", "fatal-auth-action", "string", normalizeFatalAuthActionValue(getString(cfg, "fatal-auth-action")), nil})
...
func fieldSection(apiPath string) string {
	...
	switch apiPath {
	case "port", "host", "debug", "proxy-url", "request-retry", "max-retry-interval", "force-model-prefix", "fatal-auth-enabled", "fatal-auth-action":
...
func normalizeFatalAuthActionValue(value string) string {
